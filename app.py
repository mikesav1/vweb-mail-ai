...
<head>
  ...
  <style>
    ...
    textarea { width: 100%; min-height: 230px; padding: 14px; border-radius: 12px; border: 1px solid #cbd5e1; font-family: Arial, sans-serif; font-size: 16px; box-sizing: border-box; resize: vertical; background: #f8fafc; margin-top: 6px; margin-bottom: 10px; }
    .actions form { display: inline-block; margin-right: 8px; margin-top: 8px; }
    button { border: 0; border-radius: 10px; padding: 12px 16px; cursor: pointer; font-weight: 700; font-size: 15px; }
...
def strip_quoted_text(text):
    ...
    cleaned_lines = []
    break_patterns = [r"^Den .+ skrev", r"^On .+ wrote:$", r"^Fra:$", r"^Fra:", r"^From:$", r"^From:", r"^Sendt:$", r"^Sendt:", r"^Sent:$", r"^Sent:", r"^Til:$", r"^Til:", r"^To:$", r"^To:", r"^Emne:$", r"^Emne:", r"^Subject:$", r"^Subject:", r"^Start på videresendt besked:", r"^Forwarded message", r"^[-_]{5,}$"]
    signature_patterns = [r"^Mvh\b", r"^Med venlig hilsen\b", r"^Venlig hilsen\b", r"^Best regards\b", r"^Kind regards\b", r"^Ulla Vase\b", r"^Syrenvej 5\b", r"^7200 Grindsted\b", r"^Tlf\.:", r"^E-mail:", r"^https?://", r"^<https?://"]
...
def build_date_hint(body: str) -> str:
    lower = (body or "").lower()
    mapping = {"mandag i næste uge": 0, "tirsdag i næste uge": 1, "onsdag i næste uge": 2, "torsdag i næste uge": 3, "fredag i næste uge": 4, "lørdag i næste uge": 5, "søndag i næste uge": 6}
...
def get_product_context(recipient, subject, body):
    ...
    vinterguide_keywords = ["vinterguide","snerydning","saltning","vintertjeneste","beredskab","ruter","chauffører","platform","platforme","bruger","brugere","pris","priser","starter","pro","business","hvad kan i tilbyde","hvad tilbyder i","firma","virksomhed","system"]
...
def load_replies_by_status(statuses):
    ...
    with file_lock:
      conn = get_db_connection()
        cur = conn.cursor()
        cur.execute(f"SELECT mail_id,saved_at,sender,recipient,product_context,subject,category,requires_reply,summary,draft_reply,original_preview,status,is_new,sent_at,send_error,seen FROM replies WHERE status IN ({placeholders}) ORDER BY datetime(COALESCE(sent_at, saved_at)) DESC", tuple(statuses))
...
def save_pending_reply(mail_id, sender, recipient, product_context, subject, category, summary, reply_needed, draft_reply, original_preview, status="pending_approval"):
    ...
    with file_lock:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("INSERT INTO replies (mail_id,saved_at,sender,recipient,product_context,subject,category,requires_reply,summary,draft_reply,original_preview,status,is_new,sent_at,send_error,seen) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (
            str(mail_id),
            datetime.utcnow().replace(tzinfo=timezone.utc).isoformat().replace("+00:00","Z"),
            sender, recipient, product_context, subject, category, reply_needed, summary, draft_reply,
            original_preview[:4000], status, "ja", None, None, 0
        ))
        conn.commit()
        conn.close()
...
def update_reply_status(mail_id, new_status, sent_at=None, send_error=None):
    with file_lock:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("UPDATE replies SET status = ?, sent_at = ?, send_error = ?, is_new = 'nej', seen = 1 WHERE mail_id = ?", (new_status, sent_at, send_error, str(mail_id)))
        conn.commit()
        conn.close()
...
def get_reply_by_id(mail_id):
    with file_lock:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT mail_id,saved_at,sender,recipient,product_context,subject,category,requires_reply,summary,draft_reply,original_preview,status,is_new,sent_at,send_error,seen FROM replies WHERE mail_id = ?", (str(mail_id),))
        row = cur.fetchone()
        conn.close()
    if not row:
        return None
...
def update_reply_draft(mail_id, new_text):
    with file_lock:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("UPDATE replies SET draft_reply = ?, is_new = 'nej', seen = 1 WHERE mail_id = ?", (new_text, str(mail_id)))
        conn.commit()
        conn.close()
...
def normalize_draft_reply(draft_reply, sender):
    ...
    replacements = {"Mvh [Dit navn]":"Mvh Ulla Vase","Mvh [dit navn]":"Mvh Ulla Vase","Med venlig hilsen [Dit navn]":"Mvh Ulla Vase","Med venlig hilsen [dit navn]":"Mvh Ulla Vase","[Dit navn]":"Ulla Vase","[dit navn]":"Ulla Vase","Hej Ulla,":f"Hej {first_name},","Hej ulla,":f"Hej {first_name},","Hej Mailbot,":f"Hej {first_name},"}
    ...
    if text and "Mvh Ulla Vase" not in text:
        text = text.rstrip() + "\\n\\nMvh Ulla Vase"
    return text.replace(" Mvh Ulla Vase", "\\n\\nMvh Ulla Vase").strip()
...
def is_weak_reply(text):
    ...
    weak_patterns = ["hej kim,\\n\\nmvh ulla vase","hej,\\n\\nmvh ulla vase","hej kim,\\nmvh ulla vase","hej,\\nmvh ulla vase"]
...
def needs_offer_reply(body, subject):
    ...
    patterns = ["hvad kan i tilbyde","hvad tilbyder i","hvad er det for et firma","hvad er det for en virksomhed","hvad laver i","høre mere om hvad i kan tilbyde","høre mere om systemet","hvad kan i"]
...
def should_auto_archive(sender, subject, body):
    ...
    auto_patterns = ["no-reply","noreply","order_acknowledgment","orders.apple.com","verify your identity","verify sign-in","signin.aws","bekræft videresendelse","videresendelse af e-mails","instagram","facebook","meta business","apple store","ordrenummer","verification","confirm","password reset","reset password","support@dk.one.com","skat.dk","email-service.skat.dk","donotreply@email-service.skat.dk"]
...
- Svarudkast må ALDRIG kun være hilsen og signatur.
...
- Hvis kunden spørger bredt til virksomheden eller hvad systemet kan tilbyde, skal du forklare kort hvad VinterGuide er, hvad det bruges til, og hvem det er relevant for.
...
def send_via_resend(to_email, original_subject, draft_reply):
    ...
    signature_html = "<br><br><hr style='border:none;border-top:1px solid #ddd;'><table style='font-family: Arial, sans-serif; font-size:14px; color:#222;'><tr><td style='padding-right:15px; vertical-align:top;'><a href='https://vweb.info' target='_blank'><img src='https://vweb.info/images/vweb-logo.svg' alt='Vweb logo' width='150'></a></td><td style='vertical-align:top;'><b>Ulla Vase</b><br>Syrenvej 5<br>7200 Grindsted<br>Tlf.: 91 83 07 25<br>E-mail: <a href='mailto:ulla@vweb.info'>ulla@vweb.info</a></td></tr></table>"
...
def check_mail():
    ...
    for mail_id in recent_mail_ids:
        ...
        for response_part in msg_data:
            ...
            cleaned_body = strip_quoted_text(get_plain_text_body(msg))
            if should_auto_archive(sender, subject, cleaned_body):
                save_pending_reply(mail_id_int, sender, recipient, "automatisk", subject, "automatisk", "Systemmail / automatisk mail der ikke kræver svar.", "nej", "intet", cleaned_body, status="archived")
            ...
            try:
                ...
                if is_weak_reply(draft_reply):
                    draft_reply = normalize_draft_reply(choose_fallback_reply(sender, subject, cleaned_body, product_key), sender)
                if category in REPLY_CATEGORIES and requires_reply == "ja":
                    save_pending_reply(mail_id_int, sender, recipient, product_key, subject, category, summary, requires_reply, draft_reply, cleaned_body, status="pending_approval")
                else:
                    save_pending_reply(mail_id_int, sender, recipient, product_key, subject, category, summary or "Ingen handling nødvendig.", requires_reply, "intet", cleaned_body, status="archived")
                    update_reply_status(mail_id_int, "archived")
...
def dashboard():
    ...
    counts = get_counts()
    return render_template_string(HTML_TEMPLATE, active_items=active_items, history_items=history_items[:100], selected_item=selected_item, selected_mail_id=str(selected_item["mail_id"]) if selected_item else None, pending_count=counts["pending_approval"], approved_count=counts["approved_api"], sent_count=counts["sent"], rejected_count=counts["rejected"], archived_count=counts["archived"])
...
def send_reply(mail_id):
    ...
    try:
        ...
        send_via_resend(to_email=to_email, original_subject=item["subject"], draft_reply=item["draft_reply"])
        update_reply_status(mail_id, "sent", sent_at=datetime.utcnow().replace(tzinfo=timezone.utc).isoformat().replace("+00:00","Z"))
    except Exception as e:
        update_reply_status(mail_id, "send_failed", send_error=str(e))
    return redirect(url_for("dashboard"))
